...
motor_shaft = 7.7;	// NEMA17 motor shaft exact diameter = 5
m3_dia = 4;		// 3mm hole diameter
m3_nut_hex = 1;		// 1 for hex, 0 for square nut
m3_nut_flats = 8.5;	// normal M3 hex nut exact width = 5.5
...
pulley_t_ht = 16 - 2;	// length of toothed part of pulley, standard = 12
...
screw_d = 4.5;
ext_d = 16;
scale = 1.19047619047619;
motor_shaft_int = 5.1;


union() {
    difference() {
        cylinder(pulley_t_ht - 2, d = ext_d);
        scale([scale, scale, 1]) translate([0, 0, -2]) pulley("MXL", MXL_pulley_dia , 0.508 , 1.321 );
    }
    
    difference() {
        translate([0, 0, -motor_h]) cylinder(motor_h, d = ext_d);
        translate([0,0,-motor_h-1]) cylinder(d = motor_shaft, h=pulley_b_ht + pulley_t_ht + retainer_ht + 2, $fn=motor_shaft*4);
    }
    
    translate([motor_shaft - motor_shaft_int, -motor_shaft/2-1, -motor_h]) cube([motor_shaft - motor_shaft_int + 1, motor_shaft + 2, motor_h]);
    
    translate([-(motor_shaft - motor_shaft_int + 1) - (motor_shaft - motor_shaft_int), -motor_shaft/2-1, -motor_h]) cube([motor_shaft - motor_shaft_int + 1, motor_shaft + 2, motor_h]);
