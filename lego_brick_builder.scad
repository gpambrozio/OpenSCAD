...
difference() {
    ...
    // Front
    translate([length*BRICK_WIDTH/2, WALL_THICKNESS/2, PLATE_HEIGHT*height/2]) 
    rotate([90, 0, 0]) 
    linear_extrude(height=WALL_THICKNESS/2+1) 
    scale(0.55) 
    text("Lego Jeep", valign = "center", halign = "center");

    // Back
    translate([length*BRICK_WIDTH/2, width*BRICK_WIDTH-WALL_THICKNESS/2, PLATE_HEIGHT*height/2]) 
    rotate([0, 0, 180])
    rotate([90, 0, 0]) 
    linear_extrude(height=WALL_THICKNESS/2+1) 
    scale(0.55) 
    ...
    // Left
    translate([WALL_THICKNESS/2, width*BRICK_WIDTH/2, PLATE_HEIGHT*height/2]) 
    rotate([90, 0, -90]) 
    linear_extrude(height=WALL_THICKNESS/2+1) 
    scale(0.55) 
    text("Lego", valign = "center", halign = "center");

    // Right
    translate([length*BRICK_WIDTH-WALL_THICKNESS/2, width*BRICK_WIDTH/2, PLATE_HEIGHT*height/2]) 
    rotate([90, 0, 90]) 
    linear_extrude(height=WALL_THICKNESS/2+1) 
    scale(0.55) 
    text("Jeep", valign = "center", halign = "center");
